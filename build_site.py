...
/* The whole garden found: a lucky butterfly, one colour drawn from the flock's
   own palette. Kept in storage so it stays this visitor's colour on a return
   visit rather than rerolling every time the pill redraws. */
.htally.done .tstack{display:flex; flex-direction:column; align-items:flex-start;
  gap:5px; min-width:0;}
.htally .luck{display:flex; align-items:center; gap:7px; font-size:12px;
  font-weight:600; letter-spacing:.03em; color:#F4E9E2;}
.htally.done .luck svg{width:19px; height:19px; flex:none; transform:rotate(-8deg);}
.htally.done .luck svg path{fill:var(--bw); stroke:#FBF6F2; stroke-width:1.6;
  stroke-linejoin:round;}
.htally.done .luck svg .w2{fill:var(--bw2);}
.htally .luck b{font-weight:700; color:var(--gold);}
...
@media(max-width:600px){
  ...
  /* the lucky butterfly is the point of the line, so it stays */
  .htally.done .tstack{align-items:center;}
  .htally.done .luck svg{display:block;}
  .htally .luck{font-size:11px;}
...
(function(){
  ...
  (function(){
    ...
    /* One colour of the flock is handed to the visitor as theirs. Drawn once
       and kept, so it reads as an assignment rather than a slot machine. */
    var LUCKY_NAME = {
      gold: 'Gold', jade: 'Jade', coral: 'Coral', raspberry: 'Raspberry',
      cornflower: 'Cornflower', marigold: 'Marigold', plum: 'Plum'
    };
    function luckyStore(k, v){
      try {
        if (v){ window.localStorage.setItem(k, v); return v; }
        return window.localStorage.getItem(k);
      } catch(e){
        try {
          if (v){ window.sessionStorage.setItem(k, v); return v; }
          return window.sessionStorage.getItem(k);
        } catch(e2){ return null; }
      }
    }
    function luckyWing(){
      var keys = Object.keys(WING);
      var k = luckyStore('mlbLucky26');
      if (!k || !WING[k]){
        k = keys[Math.floor(Math.random() * keys.length)];
        luckyStore('mlbLucky26', k);
      }
      return k;
    }
    ...
    var found = {}, foundN = 0, doneShown = false, luckySent = false;
    try {
    ...
    function drawTally(){
      ...
      if (foundN >= TOTAL){
        ...
        var lk = luckyWing(), lw = WING[lk];
        tally.innerHTML = BFLY +
          '<span class="tstack">' +
          '<span class="thn">You\\'ve found the whole garden. ' +
          '<span class="han">\\u82B1\\u6708\\u60C5\\u957F</span></span>' +
          '<span class="luck" style="--bw:' + lw[0] + ';--bw2:' + lw[1] + '">' +
          BFLY + '<span>Your lucky butterfly is <b>' + LUCKY_NAME[lk] + '</b></span>' +
          '</span></span>' +
          '<a class="tgo" href="#concierge" data-tally-go="1">Try our gift matcher</a>';
        if (!luckySent){ luckySent = true; ga('garden_lucky', {lucky_colour: lk}); }
      } else {
        tally.innerHTML =
